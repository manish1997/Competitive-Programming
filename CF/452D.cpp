...
void solve(){
    ...
    si(k); si3(n[0],n[1],n[2]); si3(t[0],t[1],t[2]);
    int dp[10001]={0};
    rep(i,1,k+1) rep(j,0,3) if(i>n[j]) dp[i]=max(dp[i-n[j]]+t[j], dp[i]);
    pin(dp[k]+t[0]+t[1]+t[2]);
    //if i>n[j] means that cloth we are processing is more than
    //capacity of current machine. so (i-n[j]) was least recent
    //cloth so it's end time will be minimum and we can put new
    //cloth now
    //else we need start time for ith cloth and there is
    //ample space in jth machine. so we can start at t=0
    //that's why keeping dp[i] as it is
}
