...
player arr[100002];
vector<int> grp[200001];

void solve(){
	si3(n,w,h);
	rep(i,0,n){
		si3(arr[i].gi, arr[i].pi, arr[i].ti);
		grp[arr[i].pi-arr[i].ti+100000].pb(i);
	}
	rep(i,0,200001){
		if(grp[i].size()==0) continue;
		vector<pair<int,int> > xis,yis;
		rep(j,0,grp[i].size()){
			player curr=arr[grp[i][j]];
			if(curr.gi==1){
				xis.push_back({curr.pi, grp[i][j]});
			}
			else{
				yis.push_back({curr.pi, grp[i][j]});
			}
		}
		...
		rep(j,0,xis.size()){
			ans.pb({xis[j].first, h});
		}
		repDown(j,yis.size()-1,0){
			ans.pb({w, yis[j].first});
		}

		vector<int> toassign;

		repDown(j,yis.size()-1,0){
			toassign.pb(yis[j].second);
		}
		rep(j,0,xis.size()){
			toassign.pb(xis[j].second);
		}
		int k=0;
		rep(j,0,toassign.size()) {
			arr[toassign[j]].ansx=ans[k].f;
			arr[toassign[j]].ansy=ans[k].s;
			k++;
		}
	}
	rep(i,0,n){
		cout << arr[i].ansx << " " << arr[i].ansy << endl;
	}
